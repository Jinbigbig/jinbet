...
> 通用框架规范，项目建立之初确立的基础规则。新完善的规则与踩坑经验见 [LESSONS.md](file:///LESSONS.md)。
...
- 每次 bug 修复后需评估问题严重程度，判断是否写入 [LESSONS.md](file:///LESSONS.md)
- **严重问题（必须记录）**：导致数据错误/重复/丢失、功能失效、影响线上用户体验、涉及归一化/去重/数据流等核心逻辑、根因隐蔽易复发
- **一般问题（无需记录）**：纯样式微调、UI 文案、明显的单点 bug、一次性临时问题
- 记录格式：问题现象 + 根因 + 修复方案 + 防复发措施，归入 LESSONS.md 对应章节
