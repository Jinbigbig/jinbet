#!/usr/bin/env python3
"""从网易竞彩页面抓取所有玩法赔率（胜平负/让球/比分/总进球/半全场），转换格式后注入index.html的localStorage初始化代码。"""
...
ODDS_JSON    = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'odds_data.json')
STORAGE_KEY  = 'jinbet_odds_v738'        # 与 HTML 内的一致
BETS_KEY     = 'jinbet_bets_v737'        # 投注记录 key
# ────────────────────────────────────────────
...
EXTRA_VS_TO_DATE = {
    ...
    '民主刚果 vs 乌兹别克': '2026-06-28',
    '阿尔及利 vs 奥地利': '2026-06-28',
    '南非 vs 加拿大': '2026-06-29',
}

# 球队名别名映射：网易返回的名字 -> SCHEDULE 中的标准名字
TEAM_ALIAS = {
    '民主刚果': '刚果（金）',
    '刚果金': '刚果（金）',
    '刚果(金)': '刚果（金）',
...
LEAGUE_TEAMS = {
    '瑞超': ['赫根', '佐加顿斯', '布鲁马波', '盖斯', '马尔默', '埃夫斯堡', '哈马比', '哥德堡', '北雪平', '奥雷布洛'],
}
...
def detect_league(home, guest, jc_num=''):
    if jc_num and len(jc_num) >= 3:
        prefix = jc_num[0]
        if prefix in JC_NUM_LEAGUE:
            return JC_NUM_LEAGUE[prefix]
    for league, teams in LEAGUE_TEAMS.items():
        if home in teams or guest in teams:
            return league
    return '国际赛'

def normalize_team(name):
    """将网易球队名映射为 SCHEDULE 中的标准名字"""
    if not name:
        return name
    return TEAM_ALIAS.get(name, name)

def vs_to_date_key(vs_key, match_data=None):
    if match_data and match_data.get('date'):
        date_str = match_data['date'][:10]
        if ' vs ' in vs_key:
            h, g = vs_key.split(' vs ', 1)
            h_norm = normalize_team(h)
            g_norm = normalize_team(g)
            return date_str + '_' + h_norm + '_' + g_norm
    
    if ' vs ' in vs_key:
        h, g = vs_key.split(' vs ', 1)
        h_norm = normalize_team(h)
        g_norm = normalize_team(g)
        norm_key = f"{h_norm} vs {g_norm}"
    else:
        norm_key = vs_key

    for date, games in SCHEDULE.items():
        for g in games:
            std = g['home'] + ' vs ' + g['away']
            if std == norm_key or std == vs_key:
                return date + '_' + g['home'] + '_' + g['away']
    if vs_key in EXTRA_VS_TO_DATE:
        d = EXTRA_VS_TO_DATE[vs_key]
        h, g = vs_key.split(' vs ', 1)
        return d + '_' + normalize_team(h) + '_' + normalize_team(g)
    if norm_key != vs_key:
        # 用标准化后的 vs_key 再查一次 EXTRA
        if norm_key in EXTRA_VS_TO_DATE:
            d = EXTRA_VS_TO_DATE[norm_key]
            h, g = norm_key.split(' vs ', 1)
            return d + '_' + h + '_' + g
    return None
...
'半全场': {'胜胜':'3.85', ...}
    ...
    for m in matches_163:
        ...
        date_str = m.get('match_time', '')[:10]
        date_key = f"{date_str}_{normalize_team(h)}_{normalize_team(g)}"
        league = detect_league(h, g, m.get('jc_num', ''))
...
# ── 写入 odds_data.json（供在线 fallback 读取）───────────
def write_odds_json(odds_data):
    """
    odds_data: {"主队 vs 客队": {胜,平,负,让球,比分,...}, ...}
    写入到项目根目录的 odds_data.json，格式：
      {"updated": "...", "count": N, "data": {vs_key: {...}}}
    """
    ...
    try:
        with open(ODDS_JSON, 'w', encoding='utf-8') as f:
            json.dump(out, f, ensure_ascii=False, indent=2)
        log(f"✅ 已写入 odds_data.json（{len(odds_data)} 场，更新: {now_str}）")
        return True
    except Exception as e:
        log(f"❌ 写入 odds_data.json 失败: {e}")
        return False

# ── 注入 HTML 文件 ─────────────────────────

def inject_into_html(odds_data, match_order, clear_bets=False):
    """
    把赔率数据注入 HTML 文件：
    ...
    # ── 如果需要清空投注记录，同步清空 init() 中硬编码的 _injectedBets ──
    # （init() 里会把 _injectedBets 赋值给 bets 并写回 localStorage，
    #  导致单纯 removeItem 失效，必须同时清空这个硬编码数组）
    if clear_bets:
        marker = "var _injectedBets ="
        ib_idx = content.find(marker)
        if ib_idx != -1:
            brk_start = content.find("[", ib_idx + len(marker))
            if brk_start != -1:
                stack = 1
                i = brk_start + 1
                in_str = None
                ...
                if brk_end != -1:
                    ...
                    if semi != -1:
                        replaced_len = semi + 1 - ib_idx
                        log(f"  🗑  清空硬编码注入投注记录 (_injectedBets): 删除 {replaced_len} 字符")
                        content = content[:ib_idx] + "var _injectedBets = [];" + content[semi+1:]

    # 备份
    os.makedirs(BACKUP_DIR, exist_ok=True)
    ts = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    backup_path = os.path.join(BACKUP_DIR, f"v7.90_backup_{ts}.html")
    ...
    for k, v in odds_data.items():
        date_key = v.get('date_key')
        if date_key:
            odds_date_fmt[date_key] = {k: v for k, v in v.items() if k != 'date_key'}
        else:
            new_key = vs_to_date_key(k)
            if new_key:
                odds_date_fmt[new_key] = {k: v for k, v in v.items() if k != 'date_key'}
            else:
                log("  ⚠️  无法匹配日期: " + k)
    ...
    if clear_bets:
        clear_bets_js = (
            ...
            "    console.log('[清空] 投注记录已清空');\n"
        )
    inject_js = (
        "\n// === 网易赔率自动注入（" + now_str + " / build:" + build_id + "）===\n"
        "(function(){\n"
        "  try {\n"
        + clear_bets_js +
        "    var CURRENT_BUILD = '" + build_id + "';\n"
        "    var BUILD_KEY = '" + BUILD_KEY + "';\n"
        "    var lastBuild = localStorage.getItem(BUILD_KEY);\n"
        "    if (lastBuild && lastBuild !== CURRENT_BUILD) {\n"
        "      // 检测到新版本 → 保存并强制刷新一次（绕过浏览器缓存）\n"
        "      localStorage.setItem(BUILD_KEY, CURRENT_BUILD);\n"
        "      console.log('[自动刷新] 版本变化 ' + lastBuild + ' → ' + CURRENT_BUILD + '，强制刷新');\n"
        "      setTimeout(function() { try { location.reload(true); } catch(e) { location.reload(); } }, 30);\n"
        "      return;\n"
        "    }\n"
        "    if (!lastBuild) { localStorage.setItem(BUILD_KEY, CURRENT_BUILD); }\n"
        "    var data = " + odds_json + ";\n"
        "    var order = " + order_json + ";\n"
        "    localStorage.setItem('" + STORAGE_KEY + "', JSON.stringify(data));\n"
        "    localStorage.setItem('" + STORAGE_KEY + "_order', JSON.stringify(order));\n"
        "    console.log('[网易赔率] 已写入 localStorage，共' + Object.keys(data).length + '场 (build:' + CURRENT_BUILD + ')');\n"
        "  } catch(e) {\n"
...
def main():
    ...
    parser.add_argument('--clear-bets', action='store_true', help='同时清空所有投注记录')
    ...
    try:
        ...
        if args.test:
            ...
            for m in matches[:5]:
                ...
                log(f"  {m['jc_num']} {m['home']} vs {m['guest']} 玩法:{plays} 时间:{m.get('match_time')} 联赛:{m.get('league')}")
            log(f"  ... 共 {len(matches)} 场")
            # 也打印 vs_to_date_key 匹配情况
            odds_data, match_order = convert_to_odds_data(matches)
            for k in odds_data:
                dk = vs_to_date_key(k)
                status = "✅" if dk else "❌未匹配"
                log(f"  {status} {k} -> {dk}")
            return
        ...
        # ③+ 写入 odds_data.json（GitHub Pages 在线 fallback）
        write_odds_json(odds_data)

        # ④ 注入 HTML 文件
        log("④ 注入 HTML 文件...")
        ok = inject_into_html(odds_data, match_order, clear_bets=args.clear_bets)
