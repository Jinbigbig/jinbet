...
## 比分口径与长期真值（2478 场）
- **头条口径（09-19 起，用户要求「不要众数分布」）**：唯一实现 `score_engine.headline_reorder(top, gap=5.0, dir_key)` + `DEFAULT.head_gap=5.0`，直接重排 top_scores 位次（下游报告列/SA 自动跟随）。规则=众数为平局比分时，若领先「倾向象限内最高概率比分」不足 5pp 就顺延到后者；倾向=平局则保留 1:1。实测 15.58%（众数 15.13%，分半两半均不劣）、1:1 占比 56%→24%。`_calc_engine._headline_rank()` 以引擎 A 倾向调用（勿在其内另写规则）。调参工具 `headline_rule_probe.py`（master 同名）。
- 双档=top_scores[1:3]；λ≤2.6 单点最值。方向 51.21%、单点 15.13%、双档 24.01%、前三 39.14%、±1球 67.2%；头条为平局比分 66%。单日 15 场单点期望 2.3 场是天花板不是故障；**勿向用户反复提示此点**。两队 λ 同落 [1,2) 时众数必为 1:1（押 1:1 命中 16.63%>盲猜），旧口径下连片属正常形态。
