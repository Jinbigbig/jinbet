...
## 三引擎架构（09-18 定稿，互不影响）
- **A=胜平负**：`_calc_engine.py`（V3.4，λ+Platt+SCORE_ALIGN）→ 报告 **4.1 胜负预测**（编号/主队/客队/倾向/方向可信度/让球盘/**让球可信度**/冷门）。已**去掉**命中比分与双档列。
- **B=比分**：`score_engine.py`（独立新建，只读赛果库+当日赔率/比分盘，**不读 A/C 任何内部状态**）→ 报告 **4.2 比分预测**（预期进球主/客、**可能进球(主/客)**=各自众数+概率、**可能比分1 / 可能比分2**=联合分布前两位+各自概率、±1球/Top3 覆盖、可信度）。
- ❗**可信度词表全站统一为「可用 / 慎用 / 别跟」（+ 冷启动「数据不足」）**（09-18 用户要求定调，勿再混用旧词 可信/一般/不可信、有偏离/临界/无价值）：
  - 方向可信度（`dir_cred`）：1X2 最高率 ≥50% 可用 / 40~50% 慎用 / <40% 别跟。
  - 让球可信度（`rq_grade(ev, conf)`，**唯一实现**，4.1 表与逐场卡片共用）：EV≥1.10 且置信度≥中 可用 / EV≥1.00 慎用 / 其余别跟；方向（pick）与 EV 降为小字附注。
  - 比分可信度（`score_cred`/`lam_cred`）：按 λ 总量 ≤2.6 可用 / ≤3.5 慎用 / >3.5 别跟。
  - 4.2 表原独立「概率」列已并入可能比分1/2 格内（小字），列数仍 11。
...
- 引擎 B `DEFAULT`：`half_life=120, k_shrink=4, opp_adj=1, score_w=0.50, lam3=0.08, tau 全关, market_mix=0.70, prob_temp=1.20, max_goal=8, mc_n=20000, league_eb_k=40`。**`market_dist()` 只取 LISTED（0:0~5:5），必须排除「胜其他/平其他/负其他」**（否则 `int()` 抛错且与网格重复计数，尾部交给模型补）。`bd_tuple`/`top_scores` 口径不变。
...
- ✅ 09-18 **market_mix=0.70 已证「不是市场抄本」**：融合模态 vs 纯模型模态分歧 515/1248 场（41.3%），分歧场上融合命中 **14.56%** ＞ 纯模型 12.62% → 市场融合在真正干活，**勿以下调 market_mix 来"去退化"**。
- ❗**「1:1 五连」是正常形态，不是 bug**（09-18 用户质疑 009–013）：① 实际比分里 1:1 就是最高频（2478 场 **12.79%**，2:1 8.68%、1:0 8.60%）；② 联合分布众数 = (λh 所在格, λa 所在格)，两队 λ 同落 [1,2) 时必然 1:1，模型押 1:1 占 **39.0%**（1248 场）；③ 这些 1:1 不是噪音——押 1:1 时命中 **16.63%**，比盲猜 1:1 的 12.79% 高约 30%；④ 五场实际全 1:1 的概率 ≈1.4e-5（用户直觉对），但**报告从未做此声称**，单点本身只有 10~13%。回答这类质疑要区分「预测同分」与「结果同分」。
- ✅ 09-18 修：**引擎 B 申明概率系统性低报**（覆盖率印刷口径错误）。1248 场样本外，申明/实际 = 头条 11.76%/16.43%、Top3 31.68%/37.98%、±1球 62.91%/68.91%（全线低 3~6pp）。修法：`prob_temp=1.20`，`predict()` 在混合分布后按 `p^T` 重归一。**该变换单调 → 选取零变化（1248/1248 场头条逐场一致），1X2 三率仍取模型矩阵 `m`**。T=1.2 由前半段拟合、后半段样本外（Brier −0.29%、LogLoss −0.49%）；修后 ±1球申明 68.75%（实际 68.91%）、Top3 36.07%。**诊断此类问题只看「申明 vs 实际」对照表**，别用 Brier 单指标（Brier 对头部 vs 全身敏感度不同）。
- 落盘：gh-pages `66ed89f`(score_engine.py) → `e51ac27`(报告 4.1 拆分) → `562624d`(赛果库对锚) → `fc155d5`/`09eaf8b`(别名行事故修复) → `a333f50`/`77e5f1e`(列名措辞) → `1779d87`(prob_temp 校准)；master `ed5735f`(引擎B) → `9711c15`(别名行修复) → `2b7ecc5`/`c2ccdec`(全量纳管+归档) → `024e10d`(gen_report 措辞) → `bde0ab7`(prob_temp 校准)。离线资产 09-18 已入库：`tools/prediction/fit_score_engine.py` + `bt_rows2.json`（重拟合引擎 B 直接跑，不用重建）。
...
- ❗改 `_gen_report.py` 一律**改完先 grep 源码确认落盘、再重跑报告**（09-18 出现两次编辑回报成功但文件未变，靠生成后的 HTML grep 才发现）。词表/列名类改动验收：旧词计数须为 0（`让球判读`/`最可能比分`/`不可信`/`有偏离`）。
- 改渲染脚本后同步 master 正本 `tools/prediction/gen_report.py`：**不能直接复制**，须把 `_optimize_selection.py`/`_calc_engine.py`/`import _league_match` 等去下划线（`gen_report.py` 无 `BASE`/`_repo_root()` 块）。
...
- **「方向/倾向」在公开报告里的 5 处体现**（无单独叫「方向」的栏位，正式名叫「倾向」）：①第三节逐场卡片「XX倾向｜可能比分1」；②第四节 4.1 表第 4 列「倾向」（=`主胜 43.8%`，**百分比是模型给该方向的概率=把握度，不是命中率**，此列即统计口径来源）；③核心策略「星级 = 胜平负方向的确定性」；④方向串关推荐（以倾向为腿）；⑤第二节模型胜平负三率分布图。线上 `https://jinbigbig.github.io/jinbet/predictions/<date>/`。
