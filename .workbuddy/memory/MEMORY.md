...
- ❗**选取算法唯一实现 = 根目录 `selection_algo.py`**（无下划线→可入库，2026-09-16 抽取）。`_gen_report.py`（渲染）与 `_optimize_selection.py`（每日优化回放）一律 `import selection_algo as SA`；命中判定用 `SA.pk_result`/`SA.bd_result`。**改选取逻辑只改这个文件，禁止在那两个文件里另写一份**。
- ⚠️ 抽取时发现的历史 bug：优化器旧版量级档兜底是 `'1:1'`、报告是「象限众数」→ **21.7% 场次口径分叉**，大胆档命中率被虚高估成 31.8%（**真实 11.4%**、至少一中覆盖 27%；比分精选真实 33.3%、覆盖 91%，11 天样本）。教训：任何"回放评估"必须与生产同一份代码，否则评估的是另一个算法。
