...
## 比分口径与长期真值（2478 场）
- **头条/首选口径（09-19 定稿，用户要求「不要众数分布」）**：唯一实现 `score_engine.headline_reorder(top, gap=5.0, dir_key)` + `DEFAULT.head_gap=5.0`，规则=众数为平局比分时，若领先「倾向象限内最高概率比分」不足 5pp 就顺延到后者；倾向=平局保留 1:1。实测头条 15.58%（众数 15.13%，分半两半均不劣）、1:1 占比 56%→24%。gap 6/7/10 更差（15.05/14.57/13.44）。
- ⚠️ **禁止再重排 `top_scores` 位次**：`top_scores` 恒为**联合分布纯概率降序**（口径事实，供「可能比分/Top3覆盖/±1球/双档」）；首选另存 `headline` 字段（`_calc_engine` 快照 + `_calc_results`），或由 `SA.headline_pick(cells, direction_key(m))` 现算（报告 4.2「比分预测」列、卡片、`SA.hit_pick`、`SA.pk_ref` 全走这一条）。09-19 曾因直接重排导致「可能比分1 概率 < 可能比分2」的错位（用户立刻发现）→ 已回退。
- 双档 = **除头条外概率最高的两个**（`SA.band_scores` / 卡片「比分双档」）；若改「象限内双档」实测覆盖 23.57%→18.93%，代价太大，勿改。λ≤2.6 单点最值。方向 51.21%、单点 15.13%、双档 24.01%、前三 39.14%、±1球 67.2%。单日 15 场单点期望 2.3 场是天花板不是故障；**勿向用户反复提示此点**。两队 λ 同落 [1,2) 时众数必为 1:1（押 1:1 命中 16.63%>盲猜）。调参工具 `headline_rule_probe.py`（头条）、`band_rule_probe.py`（比分列/双档）、`pk_head_probe.py`（比分精选榜），master 同名。
