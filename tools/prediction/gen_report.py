...
def big_goals_table():
    ...
    return _sub_table(
        ...
        '<br><b>为什么必须校准</b>：市场对高进球系统性定价偏高 —— 实测去水后 P(6+)、P(7+) 均值都明显高于实际，'
        '且概率越高偏离越大。'
...
单比分是 31 格划分里的一个单点，长期命中率约 <b>15%</b>，而「无脑全猜 1:1」就有约 <b>13%</b> —— <b>所以比分必须挑场次看</b>：
...
<div class="card">
  <p style="font-size:0.86rem;color:var(--muted);margin-bottom:0.7rem;">
    <b>为什么单列这一节。</b>单比分全场次平均命中只有约 <b>15%</b>，而且「无脑全场猜 1:1」就有约 <b>13%</b> ——
    所以<b>比分必须挑场次看</b>。模型不是每场都一样有把握：把「模型自评最有把握的场次」排到前面，
    单比分命中率能提到 <b>21%</b>（前 2 场 <b>21.9%</b>），取前 3 场时 <b>49% 的比赛日至少能中 1 场</b>。
    <b>本节不改变任何预测</b>，只是把最有把握的场次排到前面 ——
  ...
  <p style="font-size:0.76rem;color:var(--muted);margin-top:0.55rem;">
    排序依据 = 模型给「命中比分」的概率（小字为该场总进球 λ）。<b>对照基准：全场次约 15%</b>。
    比分可信度越高 = 该场按历史同档 λ 的实测命中率越高，越值得跟。
  </p>
</div>
'''
