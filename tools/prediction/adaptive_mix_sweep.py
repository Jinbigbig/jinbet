...
# ⚠️【2026-09-10 已查明，暂未修正】本脚本有两处与生产不同源，改动会动摇已归档结论
#   故只标注不修正；需要正确口径的对比请改用 venue_brier_check.py。
#   1) 衰减方向是反的：hist 按时间正序 append，[-10:] 的第 0 个是**最旧**一场，
#      而 wavg 把 decay^0=1 的最大权重给了第 0 个 → 越旧的比赛权重越大。
#   2) 窗口参数停留在 N=10/DECAY=0.85，生产已是 N=25/DECAY=0.96（见 window_sweep.py）。
#   说明：两处对所有对比臂一视同仁，因此「自适应衰减优于固定 w」的**方向性结论仍成立**，
#   但绝对数值（0.6303 / 0.6279）口径有偏，不可直接引用。
def lambdas(m):
