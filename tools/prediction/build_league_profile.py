...
def main():
    ...
    data = {
        ...
        "score_mix": {
            "w": 0.3,
            "k_shrink": K_FREQ,
            # 【2026-09-13 核查】0.3 是自 w=0.3 时代沿用至今的**线上真实生效值**，
            # 但引擎注释/探针/记忆长期按 0.5 处理 → 「文档口径」≠「线上口径」。
            # 实测（_mixw_probe.py，4036 场）：0.3→0.5 全样本 +21 场，时间外 272 vs 270 不显著，
            # 故维持 0.3 不动（升到 0.5 会连带触发 Platt 重拟合，收益不可证）。
            # 请勿只改注释而不改这里 —— 本值就是线上真值。
