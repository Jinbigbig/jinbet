...
if o >= 0:
    ...
    while i < len(html):
        ...
        i += 1
    raw = html[start:i + 1]
    # index.html 内联的 ODDS 是 JS 对象字面量（单引号键），并非严格 JSON；
    # 先做单引号→双引号归一，再走 JSON 解析，否则整个 ODDS 块会静默解析失败（赔率全缺）。
    _js = re.sub(r"'((?:[^'\\]|\\.)*)'",
                 lambda m: '"' + m.group(1).replace('"', '\\"') + '"', raw)
    for attempt in (raw, _js, re.sub(r",\s*([}\]])", r"\1", raw),
                    re.sub(r",\s*([}\]])", r"\1", _js)):
        try:
...
try:
    with open(os.path.join(BASE, "odds_data.json"), "r", encoding="utf-8") as f:
        ...
        # odds_data.json 为 {updated,count,data} 包裹结构，真正的键值在 data 下
        if isinstance(raw_odds, dict) and isinstance(raw_odds.get("data"), dict):
            raw_odds = raw_odds["data"]
    for m in matches:
